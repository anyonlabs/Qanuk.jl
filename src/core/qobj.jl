...
(2, 2)-element Qanuk.SparseOperator:
Underlying data ComplexF64:
 1.0 + 0.0im        ⋅     
      ⋅       -1.0 + 0.0im
...
function Base.show(io::IO, system::MultiBodySystem)
    @printf(
        io,
        "Qanuk.Multibody system with %d bodies\n",
...
function Base.show(io::IO, x::DenseOperator)
    println(io, "$(size(x.data))-element Qanuk.DenseOperator:")
...
function Base.show(io::IO, x::SparseOperator)
    println(io, "$(size(x.data))-element Qanuk.SparseOperator:")
    println(io, "Underlying data $(eltype(x.data)):")
...
function Base.show(io::IO, x::SwapLikeOperator)
    println(io, "$(size(x))-element Qanuk.SwapLikeOperator:")
...
function Base.show(io::IO, x::DiagonalOperator)
    println(io, "($(length(x.data)),$(length(x.data)))-element Qanuk.DiagonalOperator:")
...
function Base.show(io::IO, x::AntiDiagonalOperator)
    println(io, "($(length(x.data)),$(length(x.data)))-element Qanuk.AntiDiagonalOperator:")
    println(io, "Underlying data type: $(eltype(x.data)):")
    nrow=length(x.data) 
    ncol=nrow
    for i in 1:nrow
